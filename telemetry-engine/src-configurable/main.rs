...
const CHECK_STORY = {{
  ...
  'PRIV-3': {{
    headline: 'Went straight to admin \u2014 skipped valid usage',
...
function firePayload() {{
  ...
  const prereqRes = resCheckedAt !== null;
...
function addFeedEntry(entry, ts, variances, injected) {{
  ...
  let mainHtml = `
    <div class="feed-main">
      ...
      ${{isPriv && !injected && variances.length === 0 ? '<span class="feed-ok-badge">\u2714 Valid</span>' : ''}}
    </div>`;
  ...
  panel.innerHTML = ids.map(id => {{
    ...
    return `<div class="session-card ${{isAtk ? 'session-atk' : 'session-norm'}}">
      <div class="sc-top">
        ...
        <span class="sc-badge ${{isAtk ? 'sc-badge-atk' : 'sc-badge-ok'}}">${{isAtk ? 'SIMULATED' : 'VALID'}}</span>
      </div>
...
<h2>What valid looks like &mdash; Alice&rsquo;s session</h2>
...
<div class="how-it-works">
  ...
  Every legitimate admin action is preceded by an active connection, a recent login, and valid resource usage.
...
tick();
timer = setInterval(tick, 3000);
setInterval(() => {{ updateChecksPanel('sess_a1b2'); updateStatePanel(); updatePreview(); }}, 1000);
